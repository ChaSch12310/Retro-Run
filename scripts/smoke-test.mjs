...
assert.match(accountWranglerConfig, /"name"\s*:\s*"PLAYER_REPORT_EMAIL"/);
assert.match(accountWranglerConfig, /"destination_address"\s*:\s*"reports@retrorun\.win"/);
assert.match(accountWranglerConfig, /"name"\s*:\s*"ISSUE_REPORT_EMAIL"/);
assert.match(accountWranglerConfig, /"destination_address"\s*:\s*"updates@retrorun\.win"/);
assert.match(previewAccountWranglerConfig, /"name"\s*:\s*"PLAYER_REPORT_EMAIL"/);
assert.match(previewAccountWranglerConfig, /"name"\s*:\s*"ISSUE_REPORT_EMAIL"/);
assert.match(html, /id="communityButton"/);
assert.match(html, /id="accountOnlineLink"/);
assert.match(source, /Cloud Locker only works in the online game/);
...
assert.match(html, /game\.js\?v=20260917-cloud-locker-connection-guard/);
assert.match(html, /styles\.css\?v=20260917-cloud-locker-connection-guard/);
...
assert.match(html, /pocket-dynasty\.js\?v=20260908-arcade-leaderboard-board-final/);
...
assert.match(html, /Username \+ Passcode Only/);
...
assert.match(html, /id="leaderboardBoard"/);
assert.doesNotMatch(html, /id="leaderboardModal"/);
assert.match(html, /id="leaderboardCountdown"/);
assert.match(html, /id="postgameScorePanel"/);
assert.match(html, /<th scope="col">Tackles<\/th>/);
assert.match(html, /<th scope="col">Speed<\/th>/);
assert.match(html, /<th scope="col">Fans<\/th>/);
...
assert.match(source, /Create an account with only a username and passcode\. No email required\./);
...
assert.match(html, /20260908-arcade-leaderboard-board-final/);
assert.match(
  styles,
...
assert.match(html, /id="creatorRunnerSelect"/);
assert.match(html, /id="creatorRunnerNameInput"[^>]+maxlength="24"/);
...
assert.match(styles, /body\[data-game="dodgeball"\] \.soccer-keeper\.targeting\.hit/);
assert.match(styles, /@keyframes dodgeball-target-impact/);
assert.match(source, /function startDodgeballTargetImpact\(/);
assert.match(source, /classList\.toggle\("hit", fieldGoalKickMade\)/);
...
elements.get("creatorRunnerSelect").dispatch("change");
elements.get("creatorRunnerNameInput").value = "Reserve Ace";
elements.get("creatorSpeedInput").value = "88";
elements.get("creatorPowerInput").value = "77";
elements.get("creatorCutInput").value = "66";
elements.get("creatorAutoScoreInput").checked = false;
elements.get("creatorAutoScoreInput").dispatch("input");
elements.get("creatorLevelsForm").submit();
assert.equal(game.creatorAutoScore, false);
assert.equal(game.roster.find((runner) => runner.id === editedBackupId).name, "Reserve Ace");
assert.equal(game.roster.find((runner) => runner.id === editedBackupId).speed, 88);
assert.equal(game.roster.find((runner) => runner.id === editedBackupId).power, 77);
assert.equal(game.roster.find((runner) => runner.id === editedBackupId).cut, 66);
assert.equal(game.activeRunnerId, "starter");
...
assert.equal(game.fieldGoalKickMade, true);
assert.equal(elements.get("soccerKeeper").classList.contains("diving"), false);
assert.equal(elements.get("soccerKeeper").classList.contains("targeting"), true);
assert.equal(elements.get("soccerKeeper").classList.contains("hit"), true);
assert.equal(elements.get("soccerKeeper").style["--dodgeball-target-left"], "56.75%");
game.advanceFieldGoalFlight(2000);
assert.equal(elements.get("fieldGoalScene").classList.contains("shot-made"), true);
game.advanceFieldGoalFlight(3000);
assert.equal(game.gameState, "levelComplete");
...
assert.match(game.offseasonView.choices[1].description, /MOR/);
const originalRunnerId = game.activeRunnerId;
const preDraftRunnerIds = new Set(game.roster.map((runner) => runner.id));
const draftedChoice = game.offseasonView.choices.find((choice) => choice.id !== "keep");
...
assert.equal(game.rosterUnlocked, true);
assert.equal(game.roster.length, 6);
assert.equal(game.activeRunnerId, originalRunnerId);
assert.equal(elements.get("runnerGrid").children.length, 6);
assert.equal(elements.get("runnerSelectTitle").textContent, "Runner Roster");
assert.match(elements.get("runnerSelectionStatus").textContent, /6 Healthy/);
const draftedRunnerId = game.roster.find((runner) => !preDraftRunnerIds.has(runner.id)).id;
...
assert.ok(game.lastGameScores.tackleScore > 0 && game.lastGameScores.tackleScore <= 1_000_000);
assert.ok(game.lastGameScores.speedScore > 0 && game.lastGameScores.speedScore <= 1_000_000);
assert.ok(game.lastGameScores.fanScore > 0 && game.lastGameScores.fanScore <= 1_000_000);
assert.equal(elements.get("postgameTackleScore").textContent, game.lastGameScores.tackleScore.toLocaleString("en-US"));
assert.equal(game.pendingUpgrade, false);
