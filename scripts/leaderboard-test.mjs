...
const addEntry = ({
  ...
  playedAt = now - 1000, published = true, gameName = LEADERBOARD_GAMES[game] || "Holiday Game",
}) => {
  sql.exec(
    ...
    published ? now - 500 : null, game, gameName, ...scores,
  );
};
...
try {
  ...
  addEntry({ id: "holiday-published", user: "santa", game: "sleigh-bell-sprint" });
  addEntry({ id: "holiday-pending", user: "santa", game: "pumpkin-panic", published: false });
  addEntry({ id: "legacy-name", user: "legacy", gameName: "Pumpkin Panic" });
  ...
  assert.ok(initial.entries.some((entry) => entry.username === "legacy" && entry.gameName === LEADERBOARD_GAMES.gridiron));
  assert.ok(initial.entries.every((entry) => !/Holiday|Pumpkin|Sleigh/i.test(entry.gameName)));
  ...
  assert.equal(ranked.at(-1).username, "rival-20");
  ...
  assert.equal(Number(sql.exec("SELECT COUNT(*) AS count FROM leaderboard_entries").toArray()[0].count), 68);
